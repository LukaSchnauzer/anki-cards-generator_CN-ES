...
- **`resync-due-order`**: Anki asigna la posición en la cola de tarjetas NUEVAS (`due`) al momento de crear la nota, no según ningún campo — así que una tarjeta agregada suelta después del export en bloque (ej. una corrección puntual con `manual-card`, o para diagnosticar un error) queda al final de la cola aunque su `SortKey` diga que debería ir intercalada por frecuencia. Este comando recorre el mazo, calcula la posición correcta por `SortKey`, y reescribe `due` solo en las que estén fuera de orden — automatiza lo que antes se hacía a mano en el Browser (ordenar por SortKey, Ctrl+A, Cards → Reposition). Solo toca tarjetas `queue=0` (nunca repasadas) — para cualquier otra, `due` es una fecha de repaso, no una posición, y tocarlo sería un error real. Dry-run por default; `--apply` para escribir de verdad.
